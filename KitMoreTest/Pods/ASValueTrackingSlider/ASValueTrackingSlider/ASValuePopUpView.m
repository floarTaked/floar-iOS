...
const float ARROW_LENGTH_DEBUG = 13.0;
const float MIN_POPUPVIEW_WIDTH = 36.0;
const float MIN_POPUPVIEW_HEIGHT = 27.0;
const float POPUPVIEW_WIDTH_PAD_DEBUG = 1.15;
const float POPUPVIEW_HEIGHT_PAD_DEBUG = 1.1;

NSString *const fillColorAnimation_DEBUG = @"fillColor";
...
{
    [_backgroundLayer removeAnimationForKey:fillColorAnimation_DEBUG];
...
- (void)setAnimatedColors:(NSArray *)animatedColors withKeyTimes:(NSArray *)keyTimes
{
    ...
    CAKeyframeAnimation *colorAnim = [CAKeyframeAnimation animationWithKeyPath:fillColorAnimation_DEBUG];
    ...
    [_backgroundLayer addAnimation:colorAnim forKey:fillColorAnimation_DEBUG];
}
...
- (CGSize)popUpSizeForString:(NSString *)string
{
    [[_attributedString mutableString] setString:string];
    CGFloat w, h;
    w = ceilf(MAX([_attributedString size].width, MIN_POPUPVIEW_WIDTH) * POPUPVIEW_WIDTH_PAD_DEBUG);
    h = ceilf(MAX([_attributedString size].height, MIN_POPUPVIEW_HEIGHT) * POPUPVIEW_HEIGHT_PAD_DEBUG + ARROW_LENGTH_DEBUG);
...
- (void)drawPath
{
    ...
    roundedRect.size.height -= ARROW_LENGTH_DEBUG;
...
- (void)layoutSubviews
{
    ...
    CGRect textRect = CGRectMake(self.bounds.origin.x,
                                 (self.bounds.size.height-ARROW_LENGTH_DEBUG-textHeight)/2,
