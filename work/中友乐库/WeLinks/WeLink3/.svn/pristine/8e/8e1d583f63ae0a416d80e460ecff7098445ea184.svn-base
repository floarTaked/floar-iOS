...
{
    [super viewDidLoad];
    self.frameView.frame = self.view.frame;
    [self.navigationItem setLeftBarButtonItemWithWMNavigationItemStyle:WMNavigationItemStyleBack title:nil
                                                                target:self
    ...
    [self.navigationItem setTitleViewWithText:@"图片编辑"];
...
//        {
//            if(self.previewImage.size.height>320.0)
//            {
//                self.minimumScale = self.previewImage.size.height/320.0;
//            }
//            else
//            {
//                self.minimumScale = 1;
...
//        else
//        {
//            if(self.previewImage.size.width>320.0)
//            {
//                self.minimumScale = self.previewImage.size.width/320.0;
//            }
//            else
//            {
//                self.minimumScale = 1;
...
//    if(self.previewImage != self.sourceImage)
    {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            hiresCGImage = [self newScaledImage:self.sourceImage.CGImage withOrientation:self.sourceImage.imageOrientation toSize:size withQuality:kCGInterpolationLow];
...
-(void)adjustImageFrame
{
    ...
    if(rect.origin.x > self.frameView.cropRect.origin.x)
    {
        transform.tx +=  self.frameView.cropRect.origin.x - rect.origin.x;
    }
    if(rect.origin.y > self.frameView.cropRect.origin.y)
    {
        transform.ty += self.frameView.cropRect.origin.y-rect.origin.y;
    }
    if(rect.origin.x+rect.size.width < self.frameView.cropRect.origin.x + self.frameView.cropRect.size.width)
    {
        transform.tx += (self.frameView.cropRect.origin.x + self.frameView.cropRect.size.width) - (rect.size.width+rect.origin.x);
    }
    if(rect.origin.y+rect.size.height < self.frameView.cropRect.origin.y + self.frameView.cropRect.size.height)
    {
        transform.ty += (self.frameView.cropRect.origin.y + self.frameView.cropRect.size.height) - (rect.size.height+rect.origin.y);
    }
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if(self.gestureCount == 0) {
                ...
                if(scale != self.scale) {
                    ...
                    self.view.userInteractionEnabled = NO;
                    [UIView animateWithDuration:kAnimationIntervalTransform delay:0 options:UIViewAnimationOptionOverrideInheritedCurve animations:^{
                        self.imageView.transform = transform;            
