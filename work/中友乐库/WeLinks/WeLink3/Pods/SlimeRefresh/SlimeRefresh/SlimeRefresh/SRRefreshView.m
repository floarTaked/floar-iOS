...
- (id)init
{
    ...
    if (self != nil) {
        ...
        _slime = [[SRSlimeView alloc] initWithFrame:
                  CGRectMake(0.0f, 0, frame.size.width, frame.size.height)];
        _slime.startPoint = CGPointMake(frame.size.width / 2, 0);
        ...
        _refleshView.bounds = CGRectMake(0.0f, 0, kRefreshImageWidth, kRefreshImageWidth);
        ...
        _refreshSuccessTips = [[UILabel alloc] initWithFrame:_slime.frame];
        _refreshSuccessTips.textAlignment = NSTextAlignmentCenter;
        _refreshSuccessTips.text = @"刷新成功";
        _refreshSuccessTips.textColor = _slime.bodyColor;
        _refreshSuccessTips.backgroundColor = [UIColor clearColor];
        _refreshSuccessTips.hidden = YES;
        [self addSubview:_refreshSuccessTips];
    }
    return self;
}
...
- (void)setFrame:(CGRect)frame
{
    ...
    if (_slime.state == SRSlimeStateNormal) {
        _slime.frame = CGRectMake(0.0f, 0, frame.size.width, frame.size.height);
        _slime.startPoint = CGPointMake(frame.size.width / 2, _dragingHeight / 2);
    }
    _refleshView.center = _slime.startPoint;
    _activityIndicatorView.center = _slime.startPoint;
    _refreshSuccessTips.center = _refleshView.center;
}
...
- (void)setLoading:(BOOL)loading
{
    ...
    if (_loading) {
        ...
        [_activityIndicatorView stopAnimating];
        if (_showSuccessLable) {
            _refreshSuccessTips.hidden = NO;
        }
