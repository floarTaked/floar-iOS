...
//初始化属性字典，而propertys获得的字典并不是初始化的而是 变量名：属性 的字典
...
+(NSMutableDictionary*)getClassPropertyDictionary
{
    ...
    NSLog(@"testGetPropertyDictionary---%@",classDic);
    return classDic;
}
+(NSDictionary*)propertys
{
...
//            create table if not exists Book4 (id integer primary key autoincrement,title,price,author default 'Yangjf',time timestamp default current_time);
            
//            create table if not exists Book4 (SNO char(2), SNAME char(8), AGE decimal(2), SEX char(2) DEFAULT'男', DEPT char(2))
