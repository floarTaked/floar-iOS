...
{
    [super viewDidLoad];
    [self.navigationItem setTitleViewWithText:@"工作经历"];
...
- (CustomCellView *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 0)
    {
        ...
        else if(indexPath.row == 1)
        {
            ...
            postTextFiled.enabled = NO;
            [postTextFiled setPlaceholder:@"待补充"];
            if(workInfo.jobCode != nil && [workInfo.jobCode length]>0)
            {
                JobObject* job = [[LogicManager sharedInstance] getPublicObject:workInfo.jobCode type:Job];
                [postTextFiled setText:job.name];
            }
        }
