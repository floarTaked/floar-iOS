...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    switch (section)
    {
        ...
        case 3:
            ...
            break;
        case 4:
            if([UserInfo myselfInstance].blog == nil || [[UserInfo myselfInstance].blog length]<=0)
            {
                return 0;
            }
            else
    ...
    else
    {
        ...
        else if (indexPath.section == 1)
        {
            ...
            else if(indexPath.row == 2)
            {
                ...
                [self customLine:cell];
                NSString* s = [[LogicManager sharedInstance] getJobLevel:profileInfo.userInfo.jobLevel];
                if(s != nil && [s length]>0)
                {
            ...
            else if(indexPath.row == 3)
            {
                ...
                [self customLine:cell];
                NSString* s = [[LogicManager sharedInstance] getJobYear:profileInfo.userInfo.jobYear];
                if(s != nil && [s length]>0)
                {
                    cell.detailTextLabel.text = s;
                }
        ...
        else if (indexPath.section == 2)
        {
            if(indexPath.row == 1)
            {
                ...
                NSString* s = [[LogicManager sharedInstance] getEducation:profileInfo.userInfo.education];
                if(s != nil && [s length]>0)
                {
                    cell.detailTextLabel.text = s;
                }
        ...
        else if (indexPath.section == 3)
        {
            ...
            if(tags != nil && [tags length]>0)
            {
                ...
                if(count > 0)
                {
                    cell.detailTextLabel.text = [NSString stringWithFormat:@"%@等%d个标签",[arr objectAtIndex:0],count];
                }
                else
                {
                    cell.detailTextLabel.text = @"待补充";
        ...
        else if (indexPath.section == 4)
        {
            ...
            cell.detailTextLabel.text = [UserInfo myselfInstance].blog;
        }
        return cell;
    }
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (indexPath.section == 4 && indexPath.row == 1)
    {
        ...
        view.request = [NSURLRequest requestWithURL:[NSURL URLWithString:[UserInfo myselfInstance].blog]];
        [self.navigationController pushViewController:view animated:YES];
    }
    self.hidesBottomBarWhenPushed = NO;
}
