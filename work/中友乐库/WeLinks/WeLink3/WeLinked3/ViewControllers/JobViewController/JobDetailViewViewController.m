...
@property (strong, nonatomic) IBOutlet UIView *myJob;
...
//        [self.navigationController.navigationBar hideLoadingIndicator];
        ...
        else if (event == 1)
        {
            ...
            if ([self.posterID isEqualToString:[UserInfo myselfInstance].userId]) {
                self.tableView.tableFooterView = self.myJob;
            }
...
- (IBAction)deleteJob:(id)sender {
    [[NetworkEngine sharedInstance] deleteJobWithJobInfo:jobInfo Block:^(int event, id object) {
        if (1 == event) {
            [self.navigationController popViewControllerAnimated:YES];
        }else{
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"设置失败" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alertView show];
        }
    }];
}
...
- (IBAction)editJob:(id)sender
{
    PublishJobViewController *publishJob = [[PublishJobViewController alloc] init];
    publishJob.jobInfo = jobInfo;
    publishJob.isUpdateJob = YES;
    [self.navigationController pushViewController:publishJob animated:YES];
}
