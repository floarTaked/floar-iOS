...
- (id)initWithTitle:(NSString *)title message:(NSString *)message delegate:(id /*<CustomIOS7AlertViewDelegate>*/)delegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ... NS_REQUIRES_NIL_TERMINATION
{
    self = [super init];
    if (self) {
        self.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
        ...
        useMotionEffects = false;
        buttonTitles = @[cancelButtonTitle,otherButtonTitles];
...
{
    if ([_delegate respondsToSelector:@selector(customIOS7dialogButtonTouchUpInside:clickedButtonAtIndex:)]) {
        [_delegate customIOS7dialogButtonTouchUpInside:self clickedButtonAtIndex:[(UIButton *)sender tag]];
    }
...
//    [dialogContainer addSubview:lineView];
    ...
    dialogContainer.layer.cornerRadius = kCustomIOS7AlertViewCornerRadius;
