...
#import "ForwardingPostMessageItem.h"
@implementation ForwardingPostMessageItem
@synthesize data;
-(float)getHeight
{
...
-(void)buildReceiveCell:(UITableViewCell*)cell
{
    ...
    UIImageView* iconImageView = (UIImageView*)[view viewWithTag:6];
    ...
    [iconImageView setImage:[UIImage imageNamed:@"Icon120"]];
    iconImageView.frame = CGRectMake(line.frame.origin.x+10, line.frame.origin.y+10, 50, 50);
    
    
    RCLabel* lbl = (RCLabel*)[view viewWithTag:7];
    if(lbl == nil)
    {
    ...
    [str appendFormat:@"\n<p ><font color='#5A96DC' size=11>%@</font></p>",[self.data.extraData objectForKey:@"jobSalary"]];
    [str appendFormat:@"\n<p ><font color='#5A96DC' size=11>%@ %@</font></p>",[self.data.extraData objectForKey:@"city"],[self.data.extraData objectForKey:@"companys"]];
    [lbl setText:str];
    lbl.frame = CGRectMake(iconImageView.frame.origin.x+iconImageView.frame.size.width+10,
                           line.frame.origin.y+10,
                           line.frame.size.width-90, 60);
    
    
    UIImageView* indic = (UIImageView*)[view viewWithTag:8];
    if(indic == nil)
    {
    ...
    indic.frame = CGRectMake(lbl.frame.origin.x+lbl.frame.size.width, lbl.frame.origin.y +20, 7, 11);
...
-(void)buildSendCell:(UITableViewCell*)cell
{
    ...
    line.frame = CGRectMake(text.frame.origin.x-12, text.frame.origin.y+text.frame.size.height+5, text.frame.size.width+20, 0.5);
    
    
    UIImageView* iconImageView = (UIImageView*)[view viewWithTag:8];
    if(iconImageView == nil)
    {
        iconImageView = [[UIImageView alloc]init];
        iconImageView.tag = 8;
    ...
    RCLabel* lbl = (RCLabel*)[view viewWithTag:9];
    if(lbl == nil)
    {
        lbl = [[RCLabel alloc]init];
        lbl.tag = 9;
    ...
    NSMutableString* str = [[NSMutableString alloc]init];
    [str appendFormat:@"<p  lineSpacing=5 ><font face='FZLTZHK--GBK1-0' size=14>%@</font></p>",[self.data.extraData objectForKey:@"jobTitle"]];
    [str appendFormat:@"\n<p ><font color='#999999' size=11>%@</font></p>",[self.data.extraData objectForKey:@"jobSalary"]];
    [str appendFormat:@"\n<p ><font color='#999999' size=11>%@ %@</font></p>",[self.data.extraData objectForKey:@"city"],[self.data.extraData objectForKey:@"companys"]];
    [lbl setText:str];
    lbl.frame = CGRectMake(iconImageView.frame.origin.x+iconImageView.frame.size.width+10,
                           line.frame.origin.y+10, line.frame.size.width-90, 60);
    ...
    indic.frame = CGRectMake(lbl.frame.origin.x+lbl.frame.size.width, lbl.frame.origin.y +20, 12, 12);
