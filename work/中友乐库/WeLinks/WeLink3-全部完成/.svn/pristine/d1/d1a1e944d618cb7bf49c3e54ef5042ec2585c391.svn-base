...
@interface EducationListViewController ()

@end

@implementation EducationListViewController
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if(section == 0)
    {
        if(profileInfo.educationArray != nil)
        {
...
- (CustomMarginCellView *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 0)
    {
        ...
        EducationInfo* education = [profileInfo.educationArray objectAtIndex:indexPath.row];
        cell.textLabel.text = education.school;
        cell.detailTextLabel.text = education.specialty;
    ...
    else
    {
        ...
        [addButton setBackgroundImage:[UIImage imageNamed:@"addEducation"] forState:UIControlStateNormal];
        [addButton setBackgroundImage:[UIImage imageNamed:@"addEducationSelected"] forState:UIControlStateHighlighted];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section == 0)
    {
        EducationViewController* edu = [[EducationViewController alloc]initWithNibName:@"EducationViewController" bundle:nil];
        EducationInfo* info = [profileInfo.educationArray objectAtIndex:indexPath.row];
        edu.educationInfo = info;
        edu.editEnable = self.editEnable;
        [edu setCallback:^(int event, id object)
         {
             if(event == 0 && object != nil)
             {
                 //删除成功
                 if([profileInfo.educationArray containsObject:object])
                 {
                     [profileInfo.educationArray removeObject:object];
                     [table reloadData];
                 }
             }
         }];
    ...
    [edu setCallback:^(int event, id object)
     {
         if(event == 1 && object != nil)
         {
             ...
             if(![profileInfo.educationArray containsObject:object])
             {
                 [profileInfo.educationArray addObject:object];
                 [table reloadData];
             }
         }
     }];
