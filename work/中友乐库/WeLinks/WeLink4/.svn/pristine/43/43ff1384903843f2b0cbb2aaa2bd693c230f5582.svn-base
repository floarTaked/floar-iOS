...
@synthesize DBUid,userId,avatar,company,city,email,industryId,infoStep,jobCode,jobLevel,jobSalary,jobYear,name,phone,
token,createTime,education,sex,friendType,salaryStatus,phoneStatus,tags,descriptions,weiboName,weiboId,birthday,lastVisitDate,cardImage,linkedInName;
-(id)init
{
    self = [super init];
    if(self)
...
-(BOOL)synchronize:(NSString*)tableName
{
    if(self == [UserInfo myselfInstance])
    {
        ...
        [[LogicManager sharedInstance] setPersistenceData:[NSNumber numberWithInt:self.userId] withKey:USERID];
    }
