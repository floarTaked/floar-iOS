...
@synthesize cardCheckImage,cardInfo,fillInfo;
...
-(void)save:(id)sender
{
    ...
    {
        //补充个人信息
        [[NetworkEngine sharedInstance] registerSaveCard:headData card:cardInfo cardImage:cardImageData block:^(int event, id object)
         {
             if(event == 0)
             {
                 //失败
    ...
    {
        //保存名片
        [[NetworkEngine sharedInstance] saveCard:headData card:cardInfo cardImage:cardImageData block:^(int event, id object)
         {
             if(event == 0)
             {
                 //失败
             ...
             else if (event == 1)
             {
                 ...
                 [HUD hide:YES afterDelay:1 complete:^{
                     ...
                     [[NSNotificationCenter  defaultCenter]postNotificationName:@"ScanBack" object:nil];
                 }];
             }
         }];
    }
