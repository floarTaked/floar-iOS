...
- (void)viewDidLoad
{
    ...
    shareTableView.keyboardDismissMode = UIScrollViewKeyboardDismissModeOnDrag;
    
    UISwipeGestureRecognizer *leftGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(leftGotoback)];
...
-(BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"输入字数不能超过200" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alertView show];
        return NO;
    }
    else
...
-(void)textViewDidChange:(UITextView *)textView
{
    ...
    int textLength = [self unicodeLengthOfString:str];
    
    contentNumLabel.text = [NSString stringWithFormat:@"%d/200",textLength];
}
