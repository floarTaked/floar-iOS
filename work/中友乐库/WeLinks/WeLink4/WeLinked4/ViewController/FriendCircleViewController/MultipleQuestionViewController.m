...
- (void)viewDidLoad
{
    ...
    multipleTableView.keyboardDismissMode = UIScrollViewKeyboardDismissModeOnDrag;
    
    //keyBoard show or hidden
...
-(void)customAnswerCell:(UITableViewCell *)cell index:(NSIndexPath *)indexPath
{
    ...
    if (textField == nil)
    {
        ...
        textField.tag = indexPath.section * 90 + indexPath.row;
...
{
    int ownSection = textField.tag / 90;
    int ownRow = textField.tag % 90;
    NSIndexPath *index = [NSIndexPath indexPathForRow:ownRow inSection:ownSection];
    
    [self tableViewmove:multipleTableView withIndexPath:index withKeyBoardRect:keyBoradRect withView:self.view];
}
...
-(void)tableViewmove:(UITableView *)tableView withIndexPath:(NSIndexPath *)indexPath withKeyBoardRect:(CGRect)keyBoardRect withView:(UIView *)targetView
{
    CGRect cellRectInTableView = [tableView rectForRowAtIndexPath:indexPath];
    CGRect cellRectInView = [tableView convertRect:cellRectInTableView toView:targetView];
    float targetY = targetView.frame.size.height - keyBoardRect.size.height - cellRectInView.size.height;
    float shouldMoveoffSet = cellRectInView.origin.y - targetY;
    
    float offSet = tableView.contentOffset.y + shouldMoveoffSet;
    if (offSet < 0)
    {
        offSet = 0;
    }
...
#pragma mark - keyBoardFunction
-(void)keyBoardShow:(NSNotification *)note
{
    keyBoradRect = [[[note userInfo] objectForKey:UIKeyboardFrameEndUserInfoKey] CGRectValue];
}
