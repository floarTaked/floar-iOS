...
-(void)loadFromDataBase
{
    ...
    {
        if([[LogicManager sharedInstance] isMyFriend:info.userId])
        {
            [UserInfo deleteWith:NewFriendTable condition:[NSString stringWithFormat:@" where DBUid = %d and userId = %d ",
                                                           [UserInfo myselfInstance].userId,info.userId]];
        }
    }
    
    arr = [[UserDataBaseManager sharedInstance] queryWithClass:[UserInfo class]
                                                     tableName:NewFriendTable
                                                     condition:[NSString stringWithFormat:@" where DBUid=%d",[UserInfo myselfInstance].userId]];
...
-(void)customCell:(UITableViewCell*)cell info:(UserInfo*)info
{
    ...
    if(info != nil)
    {
        ...
        NSString* str = [NSString stringWithFormat:@"%@  %@",info.company,info.jobCode];
        if(str != nil)
        {
            if([str length]>20)
            {
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(buttonIndex == 1)
    {
        [UserInfo deleteWith:NewFriendTable condition:[NSString stringWithFormat:@" where DBUid=%d ",[UserInfo myselfInstance].userId]];
    }
    [self loadFromDataBase];
}
@end
