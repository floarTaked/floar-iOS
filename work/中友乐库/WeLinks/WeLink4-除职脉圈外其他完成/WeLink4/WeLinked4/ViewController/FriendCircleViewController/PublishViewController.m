...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [[NetworkEngine sharedInstance] getFeedLock:^(int event, id object) {
        if (1 == event)
        {
            ...
            [lockArray addObjectsFromArray:array];
            multipleLock = [[lockArray objectAtIndex:0] intValue];
            rateLock = [[lockArray objectAtIndex:1] intValue];
        }
        else
        {
                  ...
                  indexPath:(NSIndexPath *)index
{
    ...
    if (rightImage == nil)
    {
        rightImage = [[UIImageView alloc] initWithFrame:CGRectMake(cell.contentView.width-45, 25, 15, 18)];
        rightImage.tag = 40;
    ...
    if (index.section == 2)
    {
        ...
        if (multipleLock == 0)
        {
            rightImage.image = [UIImage imageNamed:@"img_lock_circle"];
        }
        else if (rateLock == 1)
        {
