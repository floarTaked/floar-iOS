...
- (IBAction)collectionBtnAction:(id)sender
{
    ...
    [[NetWorkEngine shareInstance] collectFeedWith:_feedId block:^(int event, id object)
    {
        if (1 == event)
        {
...
- (IBAction)removeBtnAction:(id)sender
{
    ...
    //删除网络中这条数据
    [[NetWorkEngine shareInstance] deleteFeedInMainViewWith:_feedId block:^(int event, id object)
     {
         if (1 == event)
         {
             Package *pack = (Package *)object;
...
-(void)handleReasonReturnValue:(NSNotification *)note
{
    ...
    [[NetWorkEngine shareInstance] reportFeedWith:_feedId uniqueCode:0 reportReason:str block:^(int event, id object)
    {
        if (1 == event)
        {
            Package *pack = (Package *)object;
