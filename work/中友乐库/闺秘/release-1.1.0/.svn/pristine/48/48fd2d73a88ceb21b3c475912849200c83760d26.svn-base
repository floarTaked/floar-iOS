...
{
    [[NetWorkEngine shareInstance] feedDetailInfoWith:self.signalFeed.feedId block:^(int event, id object)
    {
        if (1 == event)
        {
            ...
            if (0x06 == [returnPack getProtocalId])
            {
                [returnPack reset];
...
-(void)sendComment
{
    if (commentTextField.text != nil && [commentTextField.text length] > 0 && ![commentTextField.text isEqualToString:@" "])
    {
        ...
        [[NetWorkEngine shareInstance] supportOrCommentWith:self.signalFeed.feedId
                                                  commentId:0
                                                  operation:0x04
                                                    comment:rightCommentStr
            ...
            if (1 == event)
            {
                ...
                if ([pack isKindOfClass:[Package class]])
                {
                    ...
                    if (0 == result)
                    {
                        ...
                        /*
                         1,评论成功马上从服务器上进行拉取新数据，拉取不成功显示旧数据（看不到最新的数据，主要是自己发的数据）
                         2,本地构造数据增强用户体验能马上看到评论：但是不知道楼层id和头像id,反而会给用户带来困惑
                      ...
                      //本地构造一个
                        ...
                        comment.avatarId = arc4random()%99 + 1;
                        Comment *tempComment = [dataArray lastObject];
                        comment.floorNum = tempComment.floorNum + 1;
