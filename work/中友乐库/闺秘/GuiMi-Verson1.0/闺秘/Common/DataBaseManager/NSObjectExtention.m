...
//        }        
        ...
        for (int i = 0; i < count ; i++)
        {
            ...
            {
                [dic setValue:@"INTEGER" forKey:name];
            }
            else if ([type isEqualToString:@"TI"])
            {
                [dic setValue:@"INT" forKey:name];
...
+(BOOL)registTable:(NSString*)tableName
{
    ...
    if(!m_registed)
    {
        ...
        while ([rs next])
        {
            ...
            DLog(@"isTableOK %ld %@", (long)count,tableName);
//            NSLog(@"isTableOK %ld %@", (long)count,tableName);
            if (0 == count)
            {
                //没有数据表 直接创建
...
+(id)asynchronize:(FMResultSet*)resultSet
{
    ...
    if(dic != nil)
    {
        for(NSString* name in dic.allKeys)
        {
            ...
            {
                [obj setValue:[NSNumber numberWithLongLong:[resultSet longLongIntForColumn:name]] forKey:name];
            }
        }
    }
    return obj;
...
+(BOOL)executeWithSet:(NSString*)execute condition:(NSString*)condition tableName:(NSString*)tableName
{
    ...
    NSMutableString* sql = [NSMutableString stringWithFormat:@" update %@ %@ %@",
                            tableName,
                            execute==nil?@"":execute,
                            condition==nil?@"":condition];
